"""Move the bushy tail behind the body and reduce its end-on footprint."""
...
ASSET = "comforting_cat_v5_bushy_tail_attempt2"
WORKBENCH = Path(
...
SOURCE_BLEND = (BLEND_DIR / "comforting_cat_v5_bushy_tail_attempt1.blend").resolve()
...
checkpoint = (
    CHECKPOINT_DIR / "comforting_cat_v5_before_bushy_tail_attempt2.blend"
).resolve()
...
for vertex in tail.data.vertices:
    point = tail.matrix_world @ vertex.co
    point.y += y_shift
    vertex.co = inverse @ point
tail.data.update()

tip_points = [tip.matrix_world @ vertex.co for vertex in tip.data.vertices]
tip_inverse = tip.matrix_world.inverted()
for vertex in tip.data.vertices:
    point = tip.matrix_world @ vertex.co
    point.y += y_shift
    vertex.co = tip_inverse @ point
tip.data.update()
...
scene.name = "Comforting_Cat_V5_BushyTail_A2"
scene["comforting_cat_target_mode"] = "TRUE_360"
scene["comforting_cat_v5_stage"] = "BUSHY_TAIL_PLACEMENT"
scene["comforting_cat_v5_geometry_attempt"] = "bushy_tail_2"
scene["comforting_cat_v5_dominant_defect"] = "tail_reading_as_third_leg_in_side_view"
scene["comforting_cat_v5_rejected_attempt"] = "bushy_tail_attempt1"
scene["comforting_cat_v5_do_not_change"] = json.dumps(
    [
        "tail X trajectory and orange volume",
        "head, face and ears",
...
report = {
    "asset": ASSET,
    "stage": "BUSHY_TAIL_PLACEMENT",
    "attempt": 2,
    "source_blend": str(SOURCE_BLEND),
    "checkpoint": str(checkpoint),
    "dominant_defect": "tail_reading_as_third_leg_in_side_view",
    "rejected_attempt": "comforting_cat_v5_bushy_tail_attempt1",
    "y_shift": y_shift,
    "vertical_factor": 1.0,
    "tip_factor": 1.0,
    "before": before,
...
bpy.ops.wm.save_as_mainfile(filepath=str(FINAL_BLEND))
print("COMFORTING_CAT_V5_BUSHY_TAIL_A2=" + json.dumps(report, ensure_ascii=False))
