...
ASSET = "landing_mascot_alive_v6_companion"
SCENE_NAME = "LandingMascotAliveV6Companion"
END_FRAME = 120
FPS = 24
WIDTH = 1200
HEIGHT = 1000
...
def validate(collection: bpy.types.Collection) -> dict:
    ...
    return {
        ...
        "dominant_fix": "replace rejected blob-with-arm architecture with a cohesive rounded helper character; v6 fixes v5 crop, tube artifacts and degenerate belly panel",
        "frame_start": 1,
        "frame_end": END_FRAME,
        "fps": FPS,
        "triangles": triangles,
...
def write_scene_graph(parts: dict) -> dict:
    graph = {
        "asset": ASSET,
        "purpose": "more polished simple living character for a landing hero after rejecting v1-v5 quality",
        "target_mode": "HYBRID_HERO",
        "original_design": True,
        "dominant_defect_from_previous_best": "v4 looked like a cheap blob with an attached arm; v5 improved design but failed on crop, tube artifacts and belly-panel QA; v6 fixes those defects",
        "parts": parts,
        "expected_contacts": [
