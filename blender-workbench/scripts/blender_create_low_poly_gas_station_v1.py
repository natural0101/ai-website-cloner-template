...
ASSET = "low_poly_gas_station_v1"
SCENE_NAME = "LowPolyGasStation_Rebuild"
COLLECTION_NAME = "ABT_LOW_POLY_GAS_STATION"
WIDTH = 1200
HEIGHT = 900
...
def build_geometry(collection: bpy.types.Collection) -> list[bpy.types.Object]:
    ...
    add(create_text_mesh("LPGS_CanopyText_GAS", "GAS", (0.0, -3.605, 2.74), 0.46, mats["white"], collection, extrude=0.02))
    for x in (-2.28, 2.28):
        for y in (-3.05, -1.45):
    ...
    add(create_text_mesh("LPGS_RoadsideText", "Roadside", (-3.6, 0.005, 2.20), 0.20, mats["white"], collection, extrude=0.018))
    add(create_text_mesh("LPGS_BuildingRoadsideText", "Roadside", (0.0, -0.365, 2.36), 0.30, mats["white"], collection, extrude=0.018))
...
def scene_graph() -> dict:
    return {
        ...
        "acceptance": {
            "required_objects": ["building", "canopy", "pump", "hose", "nozzle", "trash_bin", "fence", "traffic_cone", "tire", "roadside_sign"],
            "style": "low-poly primitive modeling, readable from front and 3/4 views",
