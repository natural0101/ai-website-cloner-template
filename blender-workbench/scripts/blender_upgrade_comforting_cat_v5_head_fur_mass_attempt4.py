"""Rebuild the cheek silhouette from the clean checkpoint with broad soft masses."""
...
ASSET = "comforting_cat_v5_head_fur_mass_attempt4"
WORKBENCH = Path(
...
SOURCE_BLEND = (BLEND_DIR / "comforting_cat_v5_pear_robe_attempt1.blend").resolve()
...
checkpoint = (
    CHECKPOINT_DIR / "comforting_cat_v5_before_head_fur_mass_attempt4.blend"
).resolve()
...
for vertex in head.data.vertices:
    ...
    # One broad cheek mass with a much weaker lower continuation. Wide
    # envelopes and no negative valleys prevent the old horizontal spikes.
    lobe = (
        0.029 * gaussian(t, 0.370, 0.135)
        + 0.012 * gaussian(t, 0.285, 0.090)
    )
    # Restrict the change to the outer silhouette while keeping the transition
    # gradual enough for the low-resolution head mesh.
    point.x += math.copysign(lobe * side_ratio**5, point.x)
...
scene.name = "Comforting_Cat_V5_HeadFurMass_A4"
scene["comforting_cat_target_mode"] = "TRUE_360"
scene["comforting_cat_v5_stage"] = "HEAD_FUR_MASS"
scene["comforting_cat_v5_geometry_attempt"] = "head_fur_mass_4"
scene["comforting_cat_v5_dominant_defect"] = "sharp_horizontal_cheek_wings"
scene["comforting_cat_v5_rejected_attempts"] = json.dumps(
    ["head_fur_mass_attempt2", "head_fur_mass_attempt3"]
)
...
report = {
    ...
    "dominant_defect": "sharp_horizontal_cheek_wings",
    "rejected_attempts": json.loads(scene["comforting_cat_v5_rejected_attempts"]),
    "broad_primary_amplitude": 0.029,
    "lower_secondary_amplitude": 0.012,
    "before": before,
    "after": after,
...
bpy.ops.wm.save_as_mainfile(filepath=str(FINAL_BLEND))
print("COMFORTING_CAT_V5_HEAD_FUR_MASS_A4=" + json.dumps(report, ensure_ascii=False))
