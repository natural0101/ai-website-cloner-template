"""Correct the mouth stem world position after the worried-expression pass."""
...
ASSET = "comforting_cat_v5_worried_expression_attempt3"
WORKBENCH = Path(
...
SOURCE_BLEND = (
    BLEND_DIR / "comforting_cat_v5_worried_expression_attempt2.blend"
).resolve()
...
checkpoint = (
    CHECKPOINT_DIR / "comforting_cat_v5_before_worried_expression_attempt3.blend"
).resolve()
...
stem_points = [stem.matrix_world @ vertex.co for vertex in stem.data.vertices]
before_center_z = sum(point.z for point in stem_points) / len(stem_points)
target_center_z = 2.7255
offset_z = target_center_z - before_center_z
matrix = stem.matrix_world.copy()
matrix.translation += Vector((0.0, 0.0, offset_z))
stem.matrix_world = matrix
bpy.context.view_layer.update()

scene = bpy.context.scene
scene.name = "Comforting_Cat_V5_WorriedExpression_A3"
scene["comforting_cat_target_mode"] = "TRUE_360"
scene["comforting_cat_v5_stage"] = "FACE_EXPRESSION"
scene["comforting_cat_v5_geometry_attempt"] = "worried_expression_3"
scene["comforting_cat_v5_dominant_defect"] = "floating_mouth_stem_from_local_origin"
scene["comforting_cat_v5_do_not_change"] = json.dumps(
    [
        "head and ears",
        "eyes and pupils from attempt2",
        "eyebrows from attempt2",
        "mouth arcs from attempt2",
        "muzzle and nose",
        "costume and lower silhouette",
        "satchel and tail",
        "materials",
...
report = {
    ...
    "dominant_defect": "floating_mouth_stem_from_local_origin",
    "rejected_attempt": "comforting_cat_v5_worried_expression_attempt2",
    "mouth_stem_before_center_z": round(float(before_center_z), 6),
    "mouth_stem_offset_z": round(float(offset_z), 6),
    "mouth_stem_target_center_z": target_center_z,
...
bpy.ops.wm.save_as_mainfile(filepath=str(FINAL_BLEND))
print("COMFORTING_CAT_V5_WORRIED_EXPRESSION_A3=" + json.dumps(report, ensure_ascii=False))
