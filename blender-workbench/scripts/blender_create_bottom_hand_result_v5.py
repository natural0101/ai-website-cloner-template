...
SCENE_NAME = "BottomHandResultV5Scene"
WIDTH = 1000
HEIGHT = 820
...
def create_cylinder_cuff(collection: bpy.types.Collection, mat: bpy.types.Material) -> bpy.types.Object:
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        radius=0.34,
        depth=0.46,
        location=(1.44, -0.45, -0.49),
        rotation=(0.0, math.radians(90), 0.0),
    )
    cuff = bpy.context.object
    cuff.name = "BHCV5_Cuff"
...
def create_bottom_hand_blockout(target: dict, collection: bpy.types.Collection, clay_mat: bpy.types.Material, cuff_mat: bpy.types.Material) -> list[bpy.types.Object]:
    ...
    parts.append(
        st.create_ellipsoid(
            name="BHCV5_PalmCup",
            location=lerp_point(palm_center, palm_heel, 0.48),
            scale=(0.58, 0.205, 0.215),
    ...
    parts.append(
        st.create_ellipsoid(
            name="BHCV5_PalmHeel",
            location=lerp_point(palm_heel, avg_root, 0.18),
            scale=(0.34, 0.13, 0.125),
    ...
    parts.append(
        st.create_ellipsoid(
            name="BHCV5_FingerRootPad",
            location=lerp_point(avg_root, palm_heel, 0.42),
            scale=(0.56, 0.110, 0.105),
    ...
    # v5: compact cup. Fingertips still come from target landmarks, but the
    # hidden roots are tucked into the palm so they read as a supportive hand.
    tip_by_id = {
        "index": target_point(target, "index_tip", -0.90),
        "middle": target_point(target, "middle_tip", -0.92),
        "ring": target_point(target, "ring_tip", -0.91),
        "little": target_point(target, "little_tip", -0.88),
    }
    cup_anchor = lerp_point(avg_root, palm_heel, 0.45)
    finger_specs = [
        (
            "BHCV5_Index",
            [
                lerp_point(cup_anchor, tip_by_id["index"], 0.12),
                lerp_point(cup_anchor, tip_by_id["index"], 0.46),
                tip_by_id["index"],
            ],
            [0.102, 0.086, 0.055],
        ),
        (
            "BHCV5_Middle",
            [
                lerp_point(cup_anchor, tip_by_id["middle"], 0.06),
                lerp_point(cup_anchor, tip_by_id["middle"], 0.43),
                tip_by_id["middle"],
            ],
            [0.108, 0.088, 0.052],
        ),
        (
            "BHCV5_Ring",
            [
                lerp_point(cup_anchor, tip_by_id["ring"], 0.04),
                lerp_point(cup_anchor, tip_by_id["ring"], 0.42),
                tip_by_id["ring"],
            ],
            [0.098, 0.078, 0.046],
        ),
        (
            "BHCV5_Little",
            [
                lerp_point(cup_anchor, tip_by_id["little"], 0.02),
                lerp_point(cup_anchor, tip_by_id["little"], 0.44),
                tip_by_id["little"],
            ],
            [0.082, 0.066, 0.040],
        ),
    ...
    add_tip("BHCV5_Thumb", thumb_points[-1], thumb_radii[-1])
    ...
    wrist_points, wrist_radii = target_line_depths(
        ...
        tip_depth=-0.62,
        radius_scale=0.84,
        root_pull=(1.50, -0.46, -0.49),
    )
    wrist_points[0] = (1.44, -0.48, -0.49)
    parts.append(
        create_tapered_tube_mesh(
            name="BHCV5_Wrist",
            points=wrist_points,
...
def main() -> dict:
    ...
    scene_graph = {
        "schema_version": "bottom-hand-result-v5",
        "asset": ASSET,
        "target_mode": "HYBRID_HERO",
        "stage": "compact target-tip cup bottom hand from bottom_hand_target_v1",
        "source_target": str(TARGET),
        "triangle_budget": 90000,
        "semantic_objects": [
            {"name": "palm", "sources": ["BHCV5_PalmCup", "BHCV5_PalmHeel", "BHCV5_FingerRootPad", "BHCV5_ThumbWebPad"]},
            {"name": "fingers", "sources": ["BHCV5_Index", "BHCV5_Middle", "BHCV5_Ring", "BHCV5_Little"]},
            {"name": "thumb", "sources": ["BHCV5_Thumb"]},
            {"name": "wrist", "sources": ["BHCV5_Wrist"]},
            {"name": "cuff", "sources": ["BHCV5_Cuff"]},
        ],
        "expected_contacts": [
            {"a": "finger roots", "b": "BHCV5_FingerRootPad/BHCV5_PalmCup", "relation": "OVERLAPS", "minimum": "visible overlap before any union"},
            {"a": "BHCV5_Thumb", "b": "BHCV5_ThumbWebPad/BHCV5_PalmCup", "relation": "OVERLAPS", "minimum": "thumb is separate but rooted"},
            {"a": "BHCV5_Wrist", "b": "BHCV5_Cuff/BHCV5_PalmCup", "relation": "OVERLAPS", "minimum": "wrist reaches cuff and palm"},
        ],
        "do_not_claim": [
            "not final",
            "not accepted",
    ...
    clay_mat = material("BHCV5_MAT_WarmHand", (1.0, 0.61, 0.12, 1.0), 0.58)
    cuff_mat = material("BHCV5_MAT_SoftPinkCuff", (1.0, 0.78, 0.88, 1.0), 0.66)
    guide_mat = material("BHCV5_MAT_GuideSphere", (0.22, 0.66, 0.11, 0.48), 0.64, alpha=0.48)
    ...
    qa = {
        ...
        "observations": [
            "V5 uses target fingertip landmarks but tucks hidden roots into a compact palm cup.",
