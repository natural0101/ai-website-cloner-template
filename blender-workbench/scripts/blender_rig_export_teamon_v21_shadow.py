...
BLEND_PATH = PROJECT_ROOT / "blender-workbench" / "artifacts" / "blend" / f"teamon_reference_{VARIANT}_shadow_rigged.blend"
GLB_PATH = PROJECT_ROOT / "public" / "models" / f"teamon_reference_{VARIANT}_shadow_hand.glb"
REST_PATH = PROJECT_ROOT / "public" / "images" / f"teamon-reference-{VARIANT}-rest.png"
PRESSED_PATH = PROJECT_ROOT / "public" / "images" / f"teamon-reference-{VARIANT}-pressed.png"
REPORT_PATH = PROJECT_ROOT / "blender-workbench" / "artifacts" / "reports" / f"teamon_reference_{VARIANT}_export_report.json"
QA_PATH = PROJECT_ROOT / "blender-workbench" / "artifacts" / "reports" / f"teamon_reference_{VARIANT}_structural_qa.json"
...
hand_group = bpy.data.objects["TEAMON_ShadowBakedRoot"]
hand_group.name = "TEAMON_HandPressGroup"
keycap = bpy.data.objects["TEAMON_Keycap"]
index_mesh = next(
    obj
    for obj in bpy.data.objects
    if obj.type == "MESH" and obj.get("shadow_index_distal")
)
...
for obj in export_meshes:
    if not obj.name.startswith("TEAMON_V"):
        continue
...
if triangles > 150000:
    raise RuntimeError(f"TEAMON v21 exceeds triangle budget: {triangles}")
...
if qa["errors"]:
    raise RuntimeError(f"TEAMON v21 structural QA errors: {qa['errors']}")
if not qa["expected_contact"]["rest_pass"] or not qa["expected_contact"]["pressed_pass"]:
    raise RuntimeError(f"TEAMON v21 contact failed: {qa['expected_contact']}")
...
report = {
    "asset": f"TEAMON {VARIANT} with baked Shadow Hand E3M5",
