...
ASSET = "landing_mascot_alive_v7_companion"
SCENE_NAME = "LandingMascotAliveV7Companion"
END_FRAME = 120
FPS = 24
WIDTH = 1200
HEIGHT = 1000
...
def validate(collection: bpy.types.Collection) -> dict:
    ...
    return {
        ...
        "dominant_fix": "replace rejected blob-with-arm architecture with a cohesive rounded helper character; v7 fixes remaining rough arm joint seams",
        "frame_start": 1,
        "frame_end": END_FRAME,
        "fps": FPS,
        "triangles": triangles,
...
def write_scene_graph(parts: dict) -> dict:
    graph = {
        "asset": ASSET,
        "purpose": "more polished simple living character for a landing hero after rejecting v1-v6 quality",
        "target_mode": "HYBRID_HERO",
        "original_design": True,
        "dominant_defect_from_previous_best": "v6 became technically clean but still showed rough arm joint intersections; v7 adds clean overlap bands and larger soft transition volumes",
        "parts": parts,
        "expected_contacts": [
