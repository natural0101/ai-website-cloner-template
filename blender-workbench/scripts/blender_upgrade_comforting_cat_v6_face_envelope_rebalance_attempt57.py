"""Contract only the facial X envelope after the accepted narrower head rebuild."""
...
ASSET = "comforting_cat_v6_face_envelope_rebalance_attempt57"
WORKBENCH = Path(
...
SOURCE_BLEND = (BLEND_DIR / "comforting_cat_v6_scarf_drape_ribbon_remediation_attempt56.blend").resolve()
...
checkpoint = (
    CHECKPOINT_DIR / "comforting_cat_v6_before_face_envelope_rebalance_attempt57.blend"
).resolve()
...
preserved_sources = [
    preserve_copy(obj, "Comforting_Cat_V6", "A57", "face_envelope_too_wide_after_head_rebuild")
    for obj in changed_objects
]

for name in mesh_names:
    contract_mesh_world_x(bpy.data.objects[name], 0.94)
for name in curve_names:
    contract_curve_world_x(bpy.data.objects[name], 0.94)

# Move only the inner/root portions of whiskers; outer tips retain the natural
# long reach seen in the reference.
...
if unchanged_hash_before != unchanged_hash_after:
    raise RuntimeError("Face pass changed locked mesh geometry or transforms")

# Y/Z bounds of every contracted object are hard-locked.
yz_max_delta = 0.0
for name in changed_names:
    for key in ("min", "max", "dimensions"):
        for axis in (1, 2):
            yz_max_delta = max(
                yz_max_delta,
                abs(before[name][key][axis] - after[name][key][axis]),
            )
if yz_max_delta > 0.002:
    raise RuntimeError(f"Face depth/height lock exceeded: {yz_max_delta}")

head_width = bounds(bpy.data.objects["V6_Head"])["dimensions"][0]
eye_width = after["V6_Eye_L"]["dimensions"][0]
left_eye_center = (after["V6_Eye_L"]["min"][0] + after["V6_Eye_L"]["max"][0]) * 0.5
right_eye_center = (after["V6_Eye_R"]["min"][0] + after["V6_Eye_R"]["max"][0]) * 0.5
...
metrics = {
    ...
    "eye_geometric_center_separation_over_head": round(
        (right_eye_center - left_eye_center) / head_width, 5
    ),
...
if not 0.180 <= metrics["eye_width_over_head"] <= 0.186:
    raise RuntimeError(f"Eye/head ratio outside target: {metrics['eye_width_over_head']}")
if not 0.342 <= metrics["eye_geometric_center_separation_over_head"] <= 0.352:
    raise RuntimeError(
        f"Eye separation/head outside target: {metrics['eye_geometric_center_separation_over_head']}"
    )
if not 0.468 <= metrics["combined_muzzle_width_over_head"] <= 0.480:
...
scene.name = "Comforting_Cat_V6_FaceEnvelopeRebalance_A57"
scene["comforting_cat_v6_stage"] = "FACE_ENVELOPE_REBALANCE"
scene["comforting_cat_v6_attempt"] = 57
scene["comforting_cat_v6_dominant_defect"] = "face_envelope_too_wide_after_head_rebuild"
scene["comforting_cat_v6_source"] = str(SOURCE_BLEND)

report = {
    "asset": ASSET,
    "stage": "FACE_ENVELOPE_REBALANCE",
    "attempt": 57,
    "source_blend": str(SOURCE_BLEND),
    "checkpoint": str(checkpoint),
    "dominant_defect": "face_envelope_too_wide_after_head_rebuild",
    ...
    "scope_lock": {
        ...
        "preserved": ["all Y/Z", "nose", "mouth", "philtrum", "head", "ears", "costume", "tail", "camera", "lights", "materials"],
    },
}
finalize_pass(
    asset=ASSET,
