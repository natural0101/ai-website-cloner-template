...
ASSET = "landing_mascot_alive_v1"
SCENE_NAME = "LandingMascotAliveV1"
END_FRAME = 120
FPS = 24
WIDTH = 1200
HEIGHT = 1000
...
def build_mascot(collection: bpy.types.Collection) -> dict:
    ...
    right_arm_pivot = create_empty(collection, "LM_RightArmPivot", (0.36, -0.01, 1.2))
    ...
    upper_r = cylinder_between(collection, "LM_RightArm_upper_segment", (0.36, -0.02, 1.2), (0.52, -0.04, 1.4), 0.09, 0.076, mats["coral"], 7)
    lower_r = cylinder_between(collection, "LM_RightArm_forearm_segment", (0.52, -0.04, 1.4), (0.59, -0.08, 1.62), 0.078, 0.064, mats["coral"], 7)
    right_shoulder = ellipsoid(collection, "LM_RightShoulder_overlap_cap", (0.37, -0.09, 1.2), (0.105, 0.06, 0.105), mats["coral"], 1, smooth=True)
    palm_r = ellipsoid(collection, "LM_RightHand_mitten_palm", (0.6, -0.1, 1.71), (0.105, 0.064, 0.112), mats["coral"], 2, smooth=True)
    finger_r1 = ellipsoid(collection, "LM_RightHand_finger_top", (0.54, -0.13, 1.79), (0.031, 0.02, 0.059), mats["coral"], 1, smooth=True)
    finger_r2 = ellipsoid(collection, "LM_RightHand_finger_mid", (0.62, -0.13, 1.8), (0.031, 0.02, 0.064), mats["coral"], 1, smooth=True)
    finger_r3 = ellipsoid(collection, "LM_RightHand_finger_side", (0.695, -0.12, 1.76), (0.029, 0.019, 0.054), mats["coral"], 1, smooth=True)
    ...
    for obj in (body, belly, collar, leg_l, leg_r, foot_l, foot_r, left_stub):
    ...
    for obj in (upper_r, lower_r, right_shoulder, palm_r, finger_r1, finger_r2, finger_r3):
...
def animate_character(
    ...
    for frame, rot_y, rot_z in (
        (1, math.radians(-2.0), math.radians(-0.5)),
        (18, math.radians(3.5), math.radians(1.5)),
        (34, math.radians(-5.0), math.radians(-1.8)),
        (50, math.radians(4.5), math.radians(1.8)),
        (66, math.radians(-4.5), math.radians(-1.4)),
        (84, math.radians(1.5), math.radians(0.7)),
        (120, math.radians(-2.0), math.radians(-0.5)),
    ):
        key(right_arm, frame, loc=(0.36, -0.01, 1.2), rot=(0.0, rot_y, rot_z), scale=(1.0, 1.0, 1.0))
...
def write_scene_graph(parts: dict) -> dict:
    graph = {
        ...
        "expected_contacts": [
            ...
            "right shoulder overlaps the body edge",
