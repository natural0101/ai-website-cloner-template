...
ASSET = "low_poly_gas_station_v3"
SCENE_NAME = "LowPolyGasStation_LookdevV3"
COLLECTION_NAME = "ABT_LOW_POLY_GAS_STATION_LOOKDEV_V3"
WIDTH = 1200
HEIGHT = 900
...
def create_lighting() -> None:
    ...
    add_area("LPGS_MoonKeySoft_not_exported", (-5.6, -5.2, 7.4), 145.0, 7.5)
    add_area("LPGS_CoolFill_not_exported", (5.2, -3.2, 3.2), 28.0, 8.0)
    add_area("LPGS_BackRim_not_exported", (3.8, 4.0, 4.8), 54.0, 5.5)

    for name, location, power, color in (
        ("LPGS_RoadsideGlow_not_exported", (-3.58, -0.55, 2.42), 18.0, (1.0, 0.66, 0.28)),
        ("LPGS_CanopyWarmWash_not_exported", (0.0, -3.18, 2.42), 32.0, (1.0, 0.70, 0.34)),
        ("LPGS_WindowWarmCore_not_exported", (0.0, -0.10, 1.48), 12.0, (1.0, 0.74, 0.45)),
    ):
...
def build_geometry(collection: bpy.types.Collection) -> list[bpy.types.Object]:
    mats = {
        "asphalt": material("LPGS_MAT_AsphaltLookdev", (0.075, 0.082, 0.086, 1.0), 0.92),
        "concrete": material("LPGS_MAT_ConcreteLookdev", (0.43, 0.44, 0.41, 1.0), 0.86),
        "wall": material("LPGS_MAT_WallPaintLookdev", (0.55, 0.53, 0.47, 1.0), 0.82),
        "roof": material("LPGS_MAT_CharcoalRoofLookdev", (0.13, 0.14, 0.15, 1.0), 0.74),
        "brand": material("LPGS_MAT_PaintedRedLookdev", (0.72, 0.20, 0.14, 1.0), 0.44),
        "trim": material("LPGS_MAT_OrangeTrimLookdev", (0.86, 0.39, 0.10, 1.0), 0.52),
        "glass": material("LPGS_MAT_TintedBlueGlassLookdev", (0.10, 0.31, 0.38, 1.0), 0.24),
        "metal": material("LPGS_MAT_DullPaintedMetalLookdev", (0.38, 0.41, 0.41, 1.0), 0.50, 0.0),
        "bare_metal": material("LPGS_MAT_BareEdgeMetalLookdev", (0.70, 0.72, 0.70, 1.0), 0.34, 1.0),
        "rubber": material("LPGS_MAT_RubberLookdev", (0.075, 0.073, 0.068, 1.0), 0.84),
        "white": material("LPGS_MAT_WarmPaintLookdev", (0.68, 0.64, 0.55, 1.0), 0.78),
        "shadow": material("LPGS_MAT_ContactShadowPainted", (0.035, 0.038, 0.038, 1.0), 0.96),
        "bulb": emissive_material("LPGS_MAT_WarmBulbGlowLookdev", (1.0, 0.62, 0.24, 1.0), strength=1.35, roughness=0.42),
        "sign_text_glow": emissive_material("LPGS_MAT_SignTextGlowLookdev", (1.0, 0.78, 0.42, 1.0), strength=0.82, roughness=0.48),
        "yellow": material("LPGS_MAT_SignYellowLookdev", (0.88, 0.66, 0.14, 1.0), 0.56),
        "green": material("LPGS_MAT_SignGreenLookdev", (0.07, 0.28, 0.22, 1.0), 0.62),
        "cone": material("LPGS_MAT_ConeOrangeLookdev", (0.86, 0.28, 0.08, 1.0), 0.62),
        "person": material("LPGS_MAT_ScalePersonLookdev", (0.16, 0.20, 0.24, 1.0), 0.70),
    }
    ...
    add(create_box("LPGS_AwningLowerShadowLip", (0.0, -0.352, 2.185), (6.46, 0.035, 0.035), mats["shadow"], collection, bevel=0.003))
    for x in (-2.55, 2.55):
    ...
    for x in (-2.08, -0.78, 0.78, 2.08):
        add(create_box(f"LPGS_FacadePanelSeam_{x:+.1f}", (x, -0.052, 1.42), (0.028, 0.028, 1.12), mats["shadow"], collection, bevel=0.002))
    for z in (0.74, 1.93):
        add(create_box(f"LPGS_FacadeHorizontalJoint_{z:.2f}", (0.0, -0.055, z), (4.86, 0.026, 0.026), mats["shadow"], collection, bevel=0.002))
    ...
    for x in (-0.61, 0.61):
        add(create_box(f"LPGS_DoorSideGasket_{x:+.1f}", (x, -0.062, 0.94), (0.035, 0.035, 1.72), mats["rubber"], collection, bevel=0.003))
    add(create_box("LPGS_DoorBottomGasket", (0.0, -0.063, 0.08), (1.20, 0.035, 0.045), mats["rubber"], collection, bevel=0.003))
    for x in (-0.20, 0.20):
    ...
    for x in (-1.72, 1.72):
        ...
        add(create_box(f"LPGS_WindowTopGasket_{x:+.1f}", (x, -0.064, 1.79), (1.36, 0.032, 0.034), mats["rubber"], collection, bevel=0.002))
        add(create_box(f"LPGS_WindowBottomGasket_{x:+.1f}", (x, -0.064, 0.90), (1.36, 0.032, 0.034), mats["rubber"], collection, bevel=0.002))
        for sx in (-0.66, 0.66):
            add(create_box(f"LPGS_WindowSideGasket_{x:+.1f}_{sx:+.1f}", (x + sx, -0.064, 1.34), (0.032, 0.032, 0.86), mats["rubber"], collection, bevel=0.002))
    ...
    add(island)
    add(create_box("LPGS_PumpIslandContactShadow", (0.0, -2.25, 0.167), (4.32, 1.34, 0.012), mats["shadow"], collection, bevel=0.035))
    ...
    add(create_box("LPGS_CanopyUndersidePanel", (0.0, -2.25, 2.565), (5.18, 1.92, 0.035), mats["white"], collection, bevel=0.012))
    for x in (-1.8, 0.0, 1.8):
        add(create_box(f"LPGS_CanopyUndersideRib_{x:+.1f}", (x, -2.25, 2.535), (0.045, 1.82, 0.035), mats["shadow"], collection, bevel=0.003))
    add(create_box("LPGS_CanopyFrontLowerLip", (0.0, -3.595, 2.48), (5.42, 0.045, 0.055), mats["shadow"], collection, bevel=0.004))
    ...
    for index, x in enumerate([-1.36, -0.98, -0.60, -0.22, 0.22, 0.60, 0.98, 1.36]):
        add(create_cylinder(f"LPGS_CanopyBulbSocket_{index:02d}", (x, -3.615, 2.50), 0.052, 0.024, mats["bare_metal"], collection, vertices=10, rotation=(math.radians(90.0), 0.0, 0.0)))
    for x in (-2.28, 2.28):
        for y in (-3.05, -1.45):
    ...
    add(create_box("LPGS_FuelPumpContactShadow", (0.0, -2.34, 0.257), (0.82, 0.62, 0.014), mats["shadow"], collection, bevel=0.025))
    add(create_box("LPGS_FuelPumpSidePanelLeft", (-0.335, -2.34, 0.88), (0.026, 0.38, 0.88), mats["shadow"], collection, bevel=0.003))
    add(create_box("LPGS_FuelPumpSidePanelRight", (0.335, -2.34, 0.88), (0.026, 0.38, 0.88), mats["shadow"], collection, bevel=0.003))
    add(create_box("LPGS_FuelPumpScreen", (0.0, -2.595, 1.08), (0.43, 0.045, 0.26), mats["glass"], collection, bevel=0.006))
    add(create_box("LPGS_FuelPumpScreenBezelTop", (0.0, -2.624, 1.245), (0.51, 0.024, 0.035), mats["rubber"], collection, bevel=0.003))
    add(create_box("LPGS_FuelPumpScreenBezelBottom", (0.0, -2.624, 0.915), (0.51, 0.024, 0.035), mats["rubber"], collection, bevel=0.003))
    for x in (-0.255, 0.255):
        add(create_box(f"LPGS_FuelPumpScreenBezelSide_{x:+.1f}", (x, -2.624, 1.08), (0.035, 0.024, 0.31), mats["rubber"], collection, bevel=0.003))
    ...
    add(create_box("LPGS_ConeContactShadow", (2.95, -3.05, 0.126), (0.66, 0.58, 0.012), mats["shadow"], collection, bevel=0.03))
    ...
    add(create_box("LPGS_TireContactShadow", (3.25, 0.08, 0.055), (0.72, 0.20, 0.012), mats["shadow"], collection, bevel=0.025))

    # Stylized ground markings and contact anchors keep the low-poly scene grounded without texture baking.
    for x in (-1.65, 1.65):
        add(create_box(f"LPGS_PumpBayLine_{x:+.1f}", (x, -2.25, 0.011), (0.075, 1.72, 0.012), mats["yellow"], collection, bevel=0.002))
    add(create_box("LPGS_FrontCurbEdgePaint", (0.0, -4.54, 0.012), (6.9, 0.055, 0.012), mats["yellow"], collection, bevel=0.002))
...
def main() -> dict:
    ...
    qa = {
        ...
        "visual_notes": [
            ...
            "V3 applies Lookdev & Lighting Patch v6 principles: reduced emitter strength, darker non-clipping albedo, night lighting hierarchy and contact anchors.",
            "Added secondary details: facade seams, window/door gaskets, canopy underside ribs, bulb sockets, pump screen bezel, ground markings and stylized contact shadow plates.",
            "Materials are split by physical category: painted panel, painted metal, tinted glass, rubber, concrete/asphalt, bare metal and emissive diffuser.",
