"""Corrective TRUE_360 eye pass after rejecting the attached-lid attempt."""
...
ASSET = "comforting_cat_v5_eye_integration_attempt2"
WORKBENCH = Path(
...
SOURCE_BLEND = (
    BLEND_DIR / "comforting_cat_v5_eye_integration_attempt1.blend"
).resolve()
...
checkpoint = (
    CHECKPOINT_DIR / "comforting_cat_v5_before_eye_integration_attempt2.blend"
).resolve()
...
removed = []
for name in ("Cat_UpperLid_L", "Cat_UpperLid_R"):
...
for side, suffix in ((-1.0, "L"), (1.0, "R")):
    ...
    # Let the skull naturally occlude the side/top edges.
    set_world_location(eye, (side * 0.325, -0.520, 3.025))
    set_world_location(iris, (side * 0.325, -0.542, 3.020))
    set_world_location(pupil, (side * 0.325, -0.551, 3.020))
    set_world_location(highlight, (side * 0.290, -0.557, 3.085))

    # Reduce vertical alarm and make the pupil less needle-like.
    ...
    for vertex in pupil.data.vertices:
        vertex.co.x *= 1.18
        vertex.co.z *= 0.92
    for obj in (eye, iris, pupil):
        obj.data.update()

bpy.context.view_layer.update()

scene = bpy.context.scene
scene.name = "Comforting_Cat_V5_EyeIntegration_A2"
scene["comforting_cat_target_mode"] = "TRUE_360"
scene["comforting_cat_v5_stage"] = "FACE_ORBIT_GEOMETRY"
scene["comforting_cat_v5_geometry_attempt"] = "eye_integration_2"
scene["comforting_cat_v5_dominant_defect"] = "surface_mounted_disc_eyes"
scene["comforting_cat_v5_rejected_attempt"] = (
    "attempt1_attached_orange_lid_ellipsoids"
)
...
report = {
    ...
    "rejected_attempt": "attached_orange_lid_ellipsoids",
    "removed_parts": removed,
    "eye_depth_world": {
        name: round(float(bpy.data.objects[name].matrix_world.translation.y), 6)
        for name in (
            "Cat_Eye_L",
            "Cat_Iris_L",
            "Cat_Pupil_L",
...
bpy.ops.wm.save_as_mainfile(filepath=str(FINAL_BLEND))
print("COMFORTING_CAT_V5_EYE_INTEGRATION_A2=" + json.dumps(report, ensure_ascii=False))
