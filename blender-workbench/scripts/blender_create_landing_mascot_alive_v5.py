...
ASSET = "landing_mascot_alive_v5_companion"
SCENE_NAME = "LandingMascotAliveV5Companion"
END_FRAME = 120
FPS = 24
WIDTH = 1200
HEIGHT = 1000
...
def build_mascot(collection: bpy.types.Collection) -> dict:
    ...
    belly = rounded_box(collection, "LM5_Belly_embedded_front_panel", (0.0, -0.325, 1.05), (0.54, 0.08, 0.56), 0.11, mats["cream"], segments=10)
    badge = rounded_box(collection, "LM5_Chest_small_badge", (0.0, -0.375, 1.16), (0.20, 0.035, 0.12), 0.035, mats["signal"], segments=6)
    neck = cylinder(collection, "LM5_Neck_soft_collar_overlap", (0.0, 0.0, 1.62), 0.38, 0.16, mats["body_dark"], vertices=36)
    ...
    left_rod = tube_mesh(collection, "LM5_Antenna_left_flexible_rod", [(-0.33, -0.02, 2.62), (-0.43, -0.025, 2.78), (-0.55, -0.035, 2.80)], [0.026, 0.023, 0.020], mats["body_dark"], radial_segments=10)
    right_rod = tube_mesh(collection, "LM5_Antenna_right_flexible_rod", [(0.33, -0.02, 2.62), (0.43, -0.025, 2.78), (0.55, -0.035, 2.80)], [0.026, 0.023, 0.020], mats["body_dark"], radial_segments=10)
    ...
    left_arm = tube_mesh(collection, "LM5_LeftArm_one_piece_relaxed_tube", [(-0.56, -0.04, 1.42), (-0.86, -0.09, 1.08), (-0.74, -0.12, 0.76)], [0.125, 0.115, 0.095], mats["coral"], radial_segments=16)
    right_arm = tube_mesh(collection, "LM5_RightArm_one_piece_attached_gesture_tube", [(0.56, -0.04, 1.42), (0.85, -0.10, 1.78), (0.76, -0.15, 2.10)], [0.130, 0.115, 0.095], mats["coral"], radial_segments=16)
    shoulder_l = ellipsoid(collection, "LM5_LeftShoulder_integrated_socket", (-0.55, -0.055, 1.42), (0.18, 0.14, 0.18), mats["coral"], segments=24, rings=10)
    shoulder_r = ellipsoid(collection, "LM5_RightShoulder_integrated_socket", (0.55, -0.055, 1.42), (0.18, 0.14, 0.18), mats["coral"], segments=24, rings=10)
    elbow_l = ellipsoid(collection, "LM5_LeftElbow_soft_transition", (-0.86, -0.09, 1.08), (0.13, 0.105, 0.13), mats["coral"], segments=20, rings=8)
    elbow_r = ellipsoid(collection, "LM5_RightElbow_soft_transition", (0.85, -0.10, 1.78), (0.13, 0.105, 0.13), mats["coral"], segments=20, rings=8)
    ...
    leg_l = tube_mesh(collection, "LM5_LeftLeg_single_tapered_support", [(-0.24, 0.00, 0.50), (-0.30, -0.02, 0.20)], [0.13, 0.11], mats["body_dark"], radial_segments=14)
    leg_r = tube_mesh(collection, "LM5_RightLeg_single_tapered_support", [(0.24, 0.00, 0.50), (0.32, -0.02, 0.20)], [0.13, 0.11], mats["body_dark"], radial_segments=14)
    ...
    parts = {
        ...
        "attached_arms": [left_arm.name, right_arm.name, shoulder_l.name, shoulder_r.name, elbow_l.name, elbow_r.name, hand_l.name, hand_r.name, thumb_l.name, thumb_r.name],
...
def validate(collection: bpy.types.Collection) -> dict:
    ...
    return {
        ...
        "dominant_fix": "replace rejected blob-with-arm architecture with a cohesive rounded helper character",
        "frame_start": 1,
        "frame_end": END_FRAME,
        "fps": FPS,
        "triangles": triangles,
...
def write_scene_graph(parts: dict) -> dict:
    graph = {
        "asset": ASSET,
        "purpose": "more polished simple living character for a landing hero after rejecting v1-v4 quality",
        "target_mode": "HYBRID_HERO",
        "original_design": True,
        "dominant_defect_from_previous_best": "v4 looked like a cheap blob with an attached arm; the new pass rebuilds silhouette, body language and limb contacts",
        "parts": parts,
        "expected_contacts": [
