"""Build front-weighted soft cheeks while preserving the clean rear cranium."""
...
ASSET = "comforting_cat_v5_head_fur_mass_attempt5"
WORKBENCH = Path(
...
checkpoint = (
    CHECKPOINT_DIR / "comforting_cat_v5_before_head_fur_mass_attempt5.blend"
).resolve()
...
def front_weight(y: float) -> float:
    if y <= -0.20:
        return 1.0
    if y >= 0.10:
        return 0.0
...
for vertex in head.data.vertices:
    ...
    # Broad main cheek and weaker lower continuation. The Y mask prevents any
    # deformation on the rear half of the head.
    lobe = (
        0.047 * gaussian(t, 0.410, 0.145)
        + 0.019 * gaussian(t, 0.305, 0.105)
    )
    delta = lobe * side_ratio**5 * front_weight(point.y)
    point.x += math.copysign(delta, point.x)
...
scene.name = "Comforting_Cat_V5_HeadFurMass_A5"
scene["comforting_cat_target_mode"] = "TRUE_360"
scene["comforting_cat_v5_stage"] = "HEAD_FUR_MASS"
scene["comforting_cat_v5_geometry_attempt"] = "head_fur_mass_5"
scene["comforting_cat_v5_dominant_defect"] = "cheek_deformation_wrapped_around_cranium"
scene["comforting_cat_v5_rejected_attempts"] = json.dumps(
    ["head_fur_mass_attempt2", "head_fur_mass_attempt3", "head_fur_mass_attempt4"]
)
...
report = {
    ...
    "dominant_defect": "cheek_deformation_wrapped_around_cranium",
    "rejected_attempts": json.loads(scene["comforting_cat_v5_rejected_attempts"]),
    "front_mask": {"full_at_y": -0.20, "half_at_y": -0.05, "zero_at_y": 0.10},
    "before": before,
    "after": after,
...
bpy.ops.wm.save_as_mainfile(filepath=str(FINAL_BLEND))
print("COMFORTING_CAT_V5_HEAD_FUR_MASS_A5=" + json.dumps(report, ensure_ascii=False))
