"""Accessory pass: move and reduce the satchel to the reference hip position."""
...
ASSET = "comforting_cat_v5_satchel_hip_attempt1"
WORKBENCH = Path(
...
SOURCE_BLEND = (
    BLEND_DIR / "comforting_cat_v5_tender_expression_attempt1.blend"
).resolve()
...
checkpoint = (
    CHECKPOINT_DIR / "comforting_cat_v5_before_satchel_hip_attempt1.blend"
).resolve()
...
for name in ("Cat_SatchelBag", "Cat_SatchelFlap"):
    ...
    for vertex in obj.data.vertices:
        ...
        vertex.co.z *= 0.84
    obj.data.update()
    move_world(obj, (0.16, 0.0, -0.06))

for name in (
    "Cat_SatchelHeart",
    "Cat_SatchelTasselCord_L",
    "Cat_SatchelTasselCord_R",
    "Cat_SatchelTassel_L",
    "Cat_SatchelTassel_R",
):
    move_world(bpy.data.objects[name], (0.16, 0.0, -0.06))

strap = bpy.data.objects["Cat_SatchelStrap"]
for vertex in strap.data.vertices:
    weight = max(0.0, min(1.0, (2.15 - float(vertex.co.z)) / 1.05))
    vertex.co.x += 0.16 * weight
    vertex.co.z -= 0.06 * weight
strap.data.update()
bpy.context.view_layer.update()

scene = bpy.context.scene
scene.name = "Comforting_Cat_V5_SatchelHip_A1"
scene["comforting_cat_target_mode"] = "TRUE_360"
scene["comforting_cat_v5_stage"] = "ACCESSORY_PLACEMENT"
scene["comforting_cat_v5_geometry_attempt"] = "satchel_hip_1"
scene["comforting_cat_v5_dominant_defect"] = "large_centered_box_satchel"
...
report = {
    ...
    "dominant_defect": "large_centered_box_satchel",
    "bag_offset": [0.16, 0.0, -0.06],
    "bag_scale": [0.86, 0.88, 0.84],
    "outputs": {
...
bpy.ops.wm.save_as_mainfile(filepath=str(FINAL_BLEND))
print("COMFORTING_CAT_V5_SATCHEL_HIP=" + json.dumps(report, ensure_ascii=False))
