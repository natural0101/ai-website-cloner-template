"""TRUE_360 scarf correction: seat the diagonal drape into the neck wrap."""
...
ASSET = "comforting_cat_v5_soft_scarf_attempt3"
WORKBENCH = Path(
...
SOURCE_BLEND = (BLEND_DIR / "comforting_cat_v5_soft_scarf_attempt2.blend").resolve()
...
checkpoint = (
    CHECKPOINT_DIR / "comforting_cat_v5_before_soft_scarf_attempt3.blend"
).resolve()
...
points = [drape.matrix_world @ vertex.co for vertex in drape.data.vertices]
center = sum(points, Vector()) / len(points)
inverse = drape.matrix_world.inverted()
half_width = max(abs(point.x - center.x) for point in points)
for vertex in drape.data.vertices:
    point = drape.matrix_world @ vertex.co
    # Keep the fold inside the collar width in front and 3/4.
    point.x = center.x + (point.x - center.x) * 0.88
    edge_ratio = min(1.0, abs(point.x - center.x) / (half_width * 0.88))
    # Curve both ends back around the neck instead of leaving a flat vertical edge.
    point.y += 0.17 * edge_ratio * edge_ratio
    # Reduce the aggressive diagonal and tuck the high edge below the collar top.
    point.z += 0.065 * point.x
    if point.z > 2.345:
        point.z = 2.345 + (point.z - 2.345) * 0.08
    # The side corners sit slightly lower, avoiding a rear-view silhouette spike.
    point.z -= 0.018 * edge_ratio * edge_ratio
...
scene.name = "Comforting_Cat_V5_SoftScarf_A3"
scene["comforting_cat_target_mode"] = "TRUE_360"
scene["comforting_cat_v5_stage"] = "COSTUME_SOFT_SCARF_CONTACT"
scene["comforting_cat_v5_geometry_attempt"] = "soft_scarf_3"
scene["comforting_cat_v5_dominant_defect"] = "floating_drape_edge_and_back_spike"
...
report = {
    "asset": ASSET,
    "stage": "COSTUME_SOFT_SCARF_CONTACT",
    "attempt": 3,
    "target_mode": "TRUE_360",
    "source_blend": str(SOURCE_BLEND),
    "checkpoint": str(checkpoint),
    "dominant_defect": "floating_drape_edge_and_back_spike",
    "rejected_predecessor": "comforting_cat_v5_soft_scarf_attempt2",
    "before": before,
    "after": after,
...
bpy.ops.wm.save_as_mainfile(filepath=str(FINAL_BLEND))
print("COMFORTING_CAT_V5_SOFT_SCARF_A3=" + json.dumps(report, ensure_ascii=False))
